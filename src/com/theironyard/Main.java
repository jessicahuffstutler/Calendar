...
public class Main {
    ...
    static final int SHOW_COUNT = 5; //5 per page
    ...
    public static ArrayList<Event> selectEvents(Connection conn, boolean isAsc, int offset) throws SQLException {
        ...
        String query = String.format("SELECT * FROM events ORDER BY start_date %s LIMIT ? OFFSET ?", isAsc? "ASC" : "DESC"); //left would mean true, "DESC" would mean false
    ...
    public static ArrayList<Event> selectEvents(Connection conn) throws SQLException {
        return selectEvents(conn, true, 0); //if we dont specify "isAsc" then it will default to true
    }
